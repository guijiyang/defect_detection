    ...
    def __init__(self, in_ch, out_ch):
        ...
        if 0:
            self.up = nn.Sequential(
                nn.ConvTranspose3d(in_ch * 2 // 3, in_ch *
                                   2 // 3, kernel_size=2, stride=2, padding=0),
...
class Unet_plus(nn.Module):
    ...
    def forward(self, x):
        if self.mode == 'train':  # use the whole model when training
            ...
            x = self.up2(x, x31, x3)
            x21 = self.up30(x3, x2)
            x22 = self.up31(x31, x21, x2)
            x = self.up3(x, x21, x22, x2)
            x11 = self.up40(x2, x1)
            x12 = self.up41(x21, x11, x1)
            x13 = self.up42(x22, x11, x12, x1)
            x = self.up4(x, x11, x12, x13, x1)
            # output 0 1 2
            y3 = self.outconv(x)
            # y0 = self.outconv(x11)
            # y1 = self.outconv(x12)
...
if __name__ == '__main__':
    ...
    lnet = Unet_plus(1, 1, 'test')
    # calculate model size
    print('    Total params: %.2fMB' % (sum(p.numel()
                                            for p in lnet.parameters()) / (1024.0 * 1024) * 4))
    t1 = time.time()
